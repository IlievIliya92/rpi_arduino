...
/*--------------------------------------------------------------------*/
static uint8_t translateAnalogIn(analogInput_t input);

// Enable "input" as input for the arduino board;
inline void enbAnalogInput(analogInput_t input)
{
    DDRC &= ~(_BV(translateAnalogIn(input)));
